...
import { founderExists, getFounderPage } from "@/lib/server/public-pages";
...
// Prerender every founder page at build time — the directory is small and
// stable, and static pages never touch the database on request. We scope
// to kind="founder" so investor slugs (buffett, munger, …) don't get a
// /founders/<slug> prerender — they live at /investors/<slug>.
export async function generateStaticParams() {
  try {
    const people = await db.person.findMany({
      where: { status: "active", kind: "founder" },
      select: { slug: true },
    });
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  const data = await getFounderPage(slug);
  if (!data) return { title: "Founder", robots: { index: false, follow: false } };
  ...
  return {
    title,
    description,
    alternates: { canonical: `/founders/${slug}` },
    ...
    openGraph: { title, description, type: "profile", url: `/founders/${slug}` },
...
export default async function FounderPage({ params }: Params) {
  ...
  const data = await getFounderPage(slug);
  if (!data) {
    if (await founderExists(slug)) return <Refreshing what="founder profile" />;
    notFound();
  }

  const hasPublicContent = data.counts.publicCount > 0;
  // Cross-founder links (shared themes/companies) — internal linking across
  ...
  return (
    <div>
      ...
      <section className="mt-10">
        ...
        <div className="max-w-3xl">
          {data.passages.length > 0 ? (
            ...
            <EmptyNote>
              The {data.name} collection is being indexed — letters, speeches,
              interviews and decisions, paraphrased with full source attribution.
              Explore the active collections in the meantime.
            </EmptyNote>
          )}
        </div>
      </section>
      ...
      {connections.length > 0 && (
        <section className="mt-10">
          <SectionLabel>CONNECTED FOUNDERS</SectionLabel>
          <p className="prose-reader mt-2 max-w-3xl">
            {data.name.split(" ")[0]} shares documented ground with the founders and
            investors below — themes both return to, companies both discuss. The
            strength comes from the indexed passages themselves.
          </p>
          <div className="mt-4 grid gap-x-10 gap-y-4 sm:grid-cols-2 lg:grid-cols-3">
            {connections.map((c) => {
              // Same Person row may surface at /investors or /founders.
              // We don't know the counterpart's kind here without a second
              // query, so we link by slug; for a founder audience we point
              // to the founder surface first — the investor surface still
              // works as a fallback if the slug is an investor-only row.
              return (
                <article key={c.slug} className="border-t border-border pt-3">
                  <h3 className="font-display text-base font-semibold tracking-tight">
                    <Link
                      href={`/founders/${c.slug}`}
                  ...
                  <p className="kicker mt-1">
                    ...
                    {c.sharedThemes > 0 && c.sharedCompanies > 0 && (
          ...
          },
          {
            ...
            links: (
              <div className="flex flex-wrap gap-1.5">
                ...
                <Link href="/founders" className="chip">
                  ALL FOUNDERS →
                </Link>
              </div>
            ),
